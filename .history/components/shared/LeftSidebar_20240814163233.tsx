...
const LeftSidebar = () => {
  return (
    <section className='custom-scrollbar bg-light-200 dark:bg-dark-600 light-border rounded-xl sticky h-fit left-0 top-0 flex w-fit flex-col justify-between  overflow-y-auto border-r p-3 shadow-black/20 shadow-xl dark:shadow-none max-sm:hidden lg:w-[266px]'>
      <div className='flex flex-1 flex-col gap-6'>
        {/* masih static */}
        <button
          onClick={() => (window.location.href = "/add-product")}
          className='text-dark-300 dark:text-light-900 rounded-lg bg-primary/30 text-light-900 flex items-center justify-start gap-4 bg-transparent p-4'
        >
          <MdOutlineSell />
          <p className={`base-medium max-lg:hidden`}>Add Product</p>
        </button>
      </div>
      <div className=' flex flex-col gap-3 border-t-[2px] border-black/15 mt-10 pt-10'>
        <div>
          <Link href='/login'>
